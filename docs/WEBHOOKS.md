...
| `post.published` | `PostPublished` | `post {id, slug, title, excerpt, published_at}` — once per post, ever, when it goes live: at once if published now, within a minute of its time if scheduled (`blog:announce`, `BLOG.md` §2) |
