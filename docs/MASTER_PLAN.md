...
| 4 | Base currency and launch locales | Provisionally **base BDT, USD enabled**; locales **en + bn**. Set in `api/.env` (`ORBITO_BASE_CURRENCY`, `ORBITO_SUPPORTED_LOCALES`) — change them there before Phase 4. | ⚠️ default applied, confirm before Phase 4 |
| 5 | Video hosting | Provisionally **self-hosted + YouTube/Vimeo** behind a `VideoProvider` interface added in Phase 6. | ⚠️ default applied, confirm before Phase 6 |
