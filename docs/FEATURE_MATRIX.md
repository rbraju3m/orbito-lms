...
| N6 | Webinars (standalone, registration) | — | Yes | **P15** | Members-only until P16 gives it a public site. Created, edited, published and called off from the webinars screen (P16), and SOLD — a place is a purchasable priced like a course, delivered by `GrantOrderAccess` and given back by a refund. Nothing tells registrants when one is called off |
