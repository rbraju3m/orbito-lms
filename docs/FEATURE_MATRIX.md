...
| N6 | Webinars (standalone, registration) | — | Yes | **P15** | Members-only until P16 gives it a public site. Created, edited, published and called off from the webinars screen (P16), and SOLD — a place is a purchasable priced like a course, delivered by `GrantOrderAccess` and given back by a refund. Calling one off notifies everybody holding a place — and takes the event out of their calendar and reminders, which is what makes the notice true. And from the public site a FREE event takes a guest — an email and no account, confirmed from the inbox, reminded and told by mail with a link to join or give the place up (`GUEST_REGISTRATION.md`). Paid places still need an account; a guest's attendance is not recorded |
...
| # | Feature | Tutor | Klasio | Orbito | Notes |
|---|---|---|---|---|---|
| O1 | Blog (posts, categories, tags, authors, SEO) | — (WP native) | Yes | **P16 · partial** | Posts on the academy's public site at `/a/:academy/blog`: drafts, scheduling, a cover, a search title and description, sanitised HTML, and a `post.published` webhook (`BLOG.md`). Rendered client-side by decision — pages set their own title and description tags. No categories, tags, RSS or sitemap |
| O2 | Page builder (block-based) | Via WP builders | Yes | **P16 · in progress** | Written, not yet committed (CLAUDE.md § Current phase). Pages of blocks at `/a/:academy/p/:slug` from a closed set — heading, text, image, button, chosen courses, upcoming events, latest posts, lead form — saved as one whole list, resolved with the public scopes at render. One page may be the site's front page; pages can be linked from the header (`PAGES.md`). No custom HTML, columns or revisions |
