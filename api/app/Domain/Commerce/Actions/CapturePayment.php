        ...
        foreach ($order->items as $item) {
            ...
            try {
                $this->enroll->handle(
                    $user,
                    $course,
                    // The intent Phase 9 declared for exactly this moment: it
                    // bypasses the price gate, because the price was just
                    // paid, and records the order it came from.
                    EnrollmentIntent::purchase($order->id),
                );
            } catch (EnrollmentRejected $e) {
                // Already enrolled is fine and means the grant already
                // happened. Anything else — a full course, say — is a genuine
                // problem somebody has paid for.
